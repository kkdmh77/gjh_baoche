...
+ (void)load{
    KMSwizzleMethod([self class], @selector(shouldAutorotate), @selector(bl_shouldAutorotate));
    KMSwizzleMethod([self class], @selector(supportedInterfaceOrientations), @selector(bl_supportedInterfaceOrientations));
}

- (BOOL)bl_shouldAutorotate{ // 是否支持旋转.
    if ([self isKindOfClass:NSClassFromString(@"RTRootNavigationController")]) {
        return ((RTRootNavigationController *)self).topViewController.shouldAutorotate;
    }
    
    if ([self isKindOfClass:NSClassFromString(@"RTContainerController")]) {
        return ((RTContainerController *)self).contentViewController.shouldAutorotate;
    }
    ...
    // return [self bl_shouldAutorotate];
}

- (UIInterfaceOrientationMask)bl_supportedInterfaceOrientations{ // 支持旋转的方向.
    if ([self isKindOfClass:NSClassFromString(@"RTRootNavigationController")]) {
        return [((RTRootNavigationController *)self).topViewController supportedInterfaceOrientations];
    }
    
    if ([self isKindOfClass:NSClassFromString(@"RTContainerController")]) {
        return [((RTContainerController *)self).contentViewController supportedInterfaceOrientations];
    }
    ...
    // return [self bl_supportedInterfaceOrientations];
}
