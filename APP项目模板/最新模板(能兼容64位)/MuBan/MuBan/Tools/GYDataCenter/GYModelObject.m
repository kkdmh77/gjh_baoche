...
+ (NSArray *)propertyNames:(Class)klass {
    ...
    NSArray *arr = [GYModelObject propertyNames:class_getSuperclass(klass)];
...
- (NSMutableDictionary *)toDictionary {
    ...
    for (NSString *key in [GYModelObject propertyNames:[self class]]) {
        id value = [self valueForKey:key];
        if (value && [value isKindOfClass:[GYModelObject class]]) {
            ...
            if (internalValue && [internalValue isKindOfClass:[GYModelObject class]]) {
...
- (NSDictionary *)map {
    NSArray *properties = [GYModelObject propertyNames:[self class]];
