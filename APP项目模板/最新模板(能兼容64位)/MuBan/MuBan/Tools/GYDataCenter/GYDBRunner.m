...
- (NSString *)pathForDbName:(NSString *)dbName {
    ...
    */
    ...
    // 查找文件的优先级：沙盒->bundle->如果都没有文件直接使用沙盒路径
    NSString *sandboxPath = [[FileManager getDBPath] stringByAppendingPathComponent:dbName];
    NSString *bundlePath = GetApplicationPathFileName([dbName stringByDeletingPathExtension], [dbName pathExtension]);
    
    if (IsFileExists(sandboxPath)) {
        return sandboxPath;
    } else if (IsFileExists(bundlePath)) {
        return bundlePath;
    } else {
        return sandboxPath;
    }
...
// 查询数据库版本号
- (NSUInteger)userVersion:(NSString *)dbName
{
    __block NSUInteger version = 0;
    
    if ([dbName isValidString]) {
        GYDatabaseInfo *databaseInfo = [self databaseInfoForDbName:dbName];
        [databaseInfo.databaseQueue syncInDatabase:^(FMDatabase *db) {
