...
+ (void)setTabBarSelectedIndex:(NSUInteger)index {
    SharedAppDelegate.baseTabBarController.selectedIndex = index;
}
...
+ (void)setViewControllerNavigationBarHidden:(UIViewController *)viewController hidden:(BOOL)hidden {
    if (viewController && viewController.navigationController) {
        ...
        /*
        if ([viewController isKindOfClass:[BaseNetworkViewController class]]) {
            ((BaseNetworkViewController *)viewController).scrollView.contentInset = UIEdgeInsetsMake(hidden ? 0 : -44, 0, 0, 0);
        }
        */
    }
}
