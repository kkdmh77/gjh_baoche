...
static void write_bz2(char *data, u_int data_len, NSMutableData *dataOut) {
    NSUInteger buffer_size = 10000;
    NSMutableData *buffer = [NSMutableData dataWithCapacity:buffer_size];
    bz_stream stream = { 0 };
    stream.next_out = (char *)[buffer mutableBytes];
    stream.avail_out = (u_int)buffer_size;
    stream.next_in = data;
    stream.avail_in = 8;
    int bzret;
    ...
    @try {
        do {
            ...
            [dataOut appendBytes:[buffer bytes] length:buffer_size - stream.avail_out];
...
+ (BOOL)mergeDataWithOldFilePath:(NSString *)oldPath newFilePath:(NSString *)newPath patchPath:(NSString *)patchPath
{
    if (1 == mergeData([oldPath UTF8String], [newPath UTF8String], [patchPath UTF8String]))
    {
        return YES;
    }
    return NO;
